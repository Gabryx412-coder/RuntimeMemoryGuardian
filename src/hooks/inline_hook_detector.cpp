...
InlineHookDetector::decodeRedirectTarget(rmg::core::ByteView prologue, rmg::core::Address prologueAddress) const {
    if (prologue.size() < 5) {
        return std::nullopt;
    }
...
InlineHookDetector::scan(const rmg::platform::ProcessHandle& handle,
    ...
    for (const rmg::integrity::CodeSectionInfo& section : sections) {
        const std::size_t inspectLength =
            std::min(section.size, PROLOGUE_INSPECTION_LENGTH);
        if (inspectLength == 0) {
            continue;
        }
        ...
        finding.description =
            "section '" + section.name + "' prologue redirects to " +
