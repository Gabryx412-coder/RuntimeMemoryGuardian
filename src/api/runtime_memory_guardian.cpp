...
RuntimeMemoryGuardian::RuntimeMemoryGuardian(rmg::platform::ProcessHandle handle,
    ...
    integrityChecker_ = std::make_unique<rmg::integrity::IntegrityChecker>(*scanner_, *hashProvider_);
    processMonitor_ = std::make_unique<rmg::process::ProcessMonitor>(handle_, *platformTraits_, *hashProvider_);
}
...
RuntimeMemoryGuardian::createForSelf(std::unique_ptr<rmg::integrity::IHashProvider> hashProvider) {
    ...
    return RuntimeMemoryGuardian(std::move(*handle), std::move(platformTraits), std::move(hashProvider));
}

rmg::core::Result<RuntimeMemoryGuardian>
RuntimeMemoryGuardian::createForProcess(rmg::platform::NativeProcessId processId,
                                       std::unique_ptr<rmg::integrity::IHashProvider> hashProvider) {
...
rmg::core::Result<void> RuntimeMemoryGuardian::establishBaseline() {
    ...
    auto baseline = rmg::integrity::IntegrityBaseline::create(allSections, handle_, *scanner_, *hashProvider_);
    if (!baseline) {
...
rmg::core::Result<void> RuntimeMemoryGuardian::loadBaseline(rmg::core::ByteView serializedBaseline) {
...
rmg::core::Result<std::vector<rmg::modules::ModuleInfo>> RuntimeMemoryGuardian::listModules() const {
